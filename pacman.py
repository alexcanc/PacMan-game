...
def draw_misc():
    ...
    if game_over:
        ...
        pygame.draw.rect(screen, 'dark grey', (70, 220, 760, 260), 0, 10)
        ...
        for j in range(len(level[i])):
            ...
            if level[i][j] == 6:
                pygame.draw.arc(screen, color, [(j * num2 + (num2 * 0.5)), (i * num1 + (num1 * 0.5)), num2, num1],
                                PI / 2, PI, 3)
            if level[i][j] == 7:
                pygame.draw.arc(screen, color, [(j * num2 + (num2 * 0.5)), (i * num1 - (num1 * 0.4)), num2, num1], PI,
                                3 * PI / 2, 3)
            if level[i][j] == 8:
...
def check_position(centerx, centery):
    ...
    num2 = (WIDTH // 30)
    num3 = 15
    # check collision based on center x and center y +/- fudge factor
    if centerx // 30 < 29:
        if direction == 0:
...
while run:
    ...
    if startup_counter < 180 and not game_over and not game_won:
        moving = False  # prevents player from moving during startup
        startup_counter += 1
    else:
        moving = True
    ...
    if moving:
        ...
        if not clyde.dead and not clyde.in_box:
    ...
    if not powerup:
        if (player_circle.colliderect(blinky.rect) and not blinky.dead) or (player_circle.colliderect(pinky.rect) and not pinky.dead) or (player_circle.colliderect(inky.rect) and not inky.dead) or (player_circle.colliderect(clyde.rect) and not clyde.dead):
            if lives > 0:
               lives -= 1
               startup_counter = 0
               powerup = False
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP and direction_command == 0:
            if event.key == pygame.K_RIGHT:
